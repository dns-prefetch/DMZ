---
<img src="https://raw.githubusercontent.com/dns-prefetch/DMZ/refs/heads/main/Assets/images/global/dmz-header.svg" width="100%" height="10%">

##### Published 17/03/2025 09:28:16; Revised: 18/08/2025 12:12:48

# DMZ

I tried working a basic blog on Wordpress, but after some experimenting and messing around with pages, I found that I just cannot resist the convenience of writing articles in markdown and uploading them to Github.

# Moving to WordPress (cancelled)

The articles for this site are migrating to WordPress Disco Potato Factor [Disco Potato Factory Landing Page](https://discopotatofactory.wordpress.com/home/).

Public source code and binary artefacts will remain within this repository.

---